...
class CarInterface(CarInterfaceBase):
  ...
  def apply(self, CC, CC_SP, now_nanos=None):
    ...
    new_actuators, can_sends = super().apply(CC, CC_SP, now_nanos)
    if self.CP.carFingerprint == CAR.GWM_HAVAL_H6_MK4:
      self.last_commanded_angle = float(new_actuators.steeringAngleDeg)
    return new_actuators, can_sends

  def update(self, can_packets) -> tuple[structs.CarState, structs.CarStateSP]:
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, alpha_long, is_release, docs) -> structs.CarParams:
    ...
    if alpha_long:
      ...
      # NOTE: the per-car stopping tune (vEgoStopping / vEgoStarting / stoppingDecelRate) is deprecated
      # in sunnypilot's car.capnp — the longitudinal planner now uses shared defaults. Only stopAccel
      # survives, so the MK3/MK4 creep-out values are dropped here.
      ret.stopAccel = -0.75
